...
const EMPTY_STYLE: Omit<RewriteStyle, 'id'> = { name: '', prompt: '' };

export const StyleManagerModal: React.FC<StyleManagerModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSelectStyle = (style: RewriteStyle) => {
  ...
  const handleSave = async () => {
    ...
    try {
        if (selectedStyle) {
          await updateStyle(selectedStyle.id, formData);
          showToast("Style updated successfully!");
        } else {
          await addStyle(formData);
          showToast("Style added successfully!");
        }
        setFormData(EMPTY_STYLE);
        setSelectedStyle(null);
    } catch(err) {
        showToast("Failed to save style.");
        console.error(err);
    } finally {
        setIsSaving(false);
    }
  ...
  const handleDelete = async (styleId: string) => {
    try {
        ...
        showToast("Failed to delete style.");
        console.error(err);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" aria-modal="true" role="dialog" onClick={onClose}>
      <div className="bg-brand-surface rounded-lg shadow-2xl w-full max-w-4xl m-4 h-[70vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center p-4 border-b border-brand-bg">
          <h2 className="text-xl font-bold text-brand-text">Manage Rewrite Styles</h2>
        ...
        <div className="flex flex-grow min-h-0">
          ...
          <div className="w-1/3 border-r border-brand-bg p-2 flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto pr-1">
                {styles.length === 0 ? (
                    <p className="text-center text-sm text-brand-text-secondary mt-8">No custom styles yet.</p>
                ) : (
          ...
          <div className="w-2/3 p-4 flex flex-col space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., 'More Humorous' or 'Formal Tone'"
            ...
            <div className="flex-grow flex flex-col">
              ...
              <textarea
                ...
                onChange={handleFormChange}
                placeholder="e.g., 'Rewrite the script to be funnier. Add more jokes and witty dialogue.'"
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-3">
                <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 w-28 text-sm font-medium rounded-md text-brand-text-inverse bg-brand-primary hover:bg-opacity-90 disabled:opacity-50 transition-colors">
